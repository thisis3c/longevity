...
class backupbuddy_core {
	...
	public static function cronback_test() {
		...
		// Delete Previous temp crons in the event they weren't cleared.
		wp_clear_scheduled_hook( 'bub_test_cronback' );

		// Create temp cron for testing that crons are actually scheduled.
		wp_schedule_single_event( time() - 1, 'bub_test_cronback' );

		// Call actual cron via loopback to test if it triggered
		$cron_request = array(
			'url'  => $cronURL,
			'key'  => $doing_wp_cron,
			'args' => array(
		...
		if ( is_wp_error( $response ) ) { // Loopback failed. Some kind of error.
			...
			require_once( pb_backupbuddy::plugin_path() . '/classes/fileoptions.php' );
			$fileoptions_obj = new pb_backupbuddy_fileoptions( backupbuddy_core::getLogDirectory() . 'fileoptions/cronback-test.txt', true, true, false );
			$cronstamp = &$fileoptions_obj->options;
			$now       = microtime(true);
			if ( $cronstamp && ( $now - $cronstamp < 5 ) ) {
				// If here, the cron was executed in the last 5 seconds and most likely good to go.
				...
				@unlink( backupbuddy_core::getLogDirectory() . 'fileoptions/cronback-test.txt' );
				return true;
			} else {
				// If we made it here, the script isn't blocked, but the crons aren't being executed.
				$error = 'Your wp-cron.php is reachable by the server but something is preventing it from executing code properly. Coming Soon plugins and Basic Authentication often break this.';
				$backupbuddy_cronback_details = $error;
				pb_backupbuddy::status( 'error', $error );
				return $error;
			}
